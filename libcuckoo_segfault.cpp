...
void RunWorkerThread(const int &thread_id, cuckoohash_map<int64_t, std::vector<int64_t>> *my_map, const std::vector<int> config) {
  PinToCore(thread_id);
  ...
  while (true) {
    ...
    // } else if (rand_num % 100 < config[0] + config[1] + config[2]){
      ...
      bool ret = my_map->insert(my_id, std::vector<int64_t>(1, 100));
      // if (ret == false) {
...
void RunWorkload(const int &thread_count, const std::vector<int> config) {
  ...
  cuckoohash_map<int64_t, std::vector<int64_t>> my_map(10000000 * thread_count);
  max_tuple_id = 1000;
  // populate.
  for (int i = 0; i < max_tuple_id; ++i) {
    my_map.insert(i, std::vector<int64_t>(1, 100));
  }
